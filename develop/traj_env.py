...
class MetaDriveTrajEnv(BaseEnv):
    ...
    def __init__(self, config: Union[dict, None] = None):
        ...
        super(MetaDriveTrajEnv, self).__init__(config)
    ...
    def _preprocess_macro_actions(self, actions: Union[np.ndarray, Dict[AnyStr, np.ndarray], int]) \
            -> Union[np.ndarray, Dict[AnyStr, np.ndarray], int]:
        data = np.array([
            [0.0, 0.0],
            [0.98468775, -0.01159951],
            [1.9695146, -0.01539587],
            [2.9560924, 0.00810025],
            [3.9443545, 0.04650213],
            [4.933532, 0.08059958],
            [5.923269, 0.10115331],
            [6.9125896, 0.10904672],
            [7.9013705, 0.11043843],
            [8.889958, 0.11121523],
            [9.877668, 0.11513573]
        ], dtype=np.float32)
        zt_data = calculate_and_combine_tensor_with_zeros(data)
        actions = zt_data #data
